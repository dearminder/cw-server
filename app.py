...
def AdminNewBuild():
	if request.method == "POST":
		...
		with zipfile.ZipFile(f"data/game/files/windows/{form['version']}/windows.zip", 'r') as zip_ref:
			zip_ref.extractall(f"data/game/files/windows/{form['version']}")
   
		#remove zip
		os.remove(f"data/game/files/windows/{form['version']}/windows.zip")
  
		
		AdminGenerateManifest(form["version"])
  
		#update version.txt
		with open("version.txt", "w") as f:
			f.write(form["version"])
